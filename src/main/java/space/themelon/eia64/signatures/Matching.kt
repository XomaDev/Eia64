...
object Matching {
    ...
    fun numericOrChar(first: Expression, second: Expression) = numericOrChar(first) && numericOrChar(second)
    fun numericOrChar(expression: Expression) = expression.sig().let {
        it == Sign.INT || it == Sign.FLOAT || it == Sign.CHAR
    }
